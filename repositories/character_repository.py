...
def get_all_characters(db: Session):
    return db.query(Character).order_by(Character.level.asc()).all()

def get_character_by_id(db: Session, character_id: uuid.UUID):
    return db.query(Character).filter_by(id=character_id).first()

def get_character_by_order(db: Session, level: int):
    return db.query(Character).filter_by(level=level).first()
